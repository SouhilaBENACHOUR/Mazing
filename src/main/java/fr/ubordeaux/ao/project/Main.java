...
public class Main {
    ...
    private static void startLevelProgressionChecker() {
        Thread progressionThread = new Thread(() -> {
            ...
            while (true) {
                try {
                    ...
                    if (System.currentTimeMillis() % 3000 < 300) {
                        System.out.println("🔍 CHECK: level=" + currentLevel +
                    ...
                    if (levelComplete && !loading) {
                        ...
                        System.out.println("\n⚠️ NIVEAU COMPLET DÉTECTÉ !");
                        System.out.println("📍 currentLevel AVANT = " + currentLevel);
                        ...
                        System.out.println("📍 currentLevel APRÈS = " + currentLevel);
                        System.out.println("❓ currentLevel <= 3 ? " + (currentLevel <= 3));

                        if (currentLevel <= 3) {
                            System.out.println("➡️ BRANCHE: Passage au niveau suivant");
                            ...
                            if (gameModel.getMaze() != null) {
                                System.out.println("🎨 Redessin du niveau " + currentLevel);
                                gameView.drawMaze(gameModel.getMaze());
                                gameView.drawItems(gameModel.getMaze());
                            } else {
                                System.err.println("❌ ERREUR: maze est null après chargement !");
                            }
                    ...
                    System.err.println("⚠️ Thread interrompu");
                    break;
                } catch (Exception e) {
            ...
            System.out.println("🛑 Thread de progression terminé");
        });
